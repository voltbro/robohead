...
class ASRControl(object):
	def __init__(self):
		...
		self._grammar_data_pub = rospy.Publisher("/head/cmd_from_voice", String, queue_size=10)
		self._grammar_not_found_pub = rospy.Publisher('/head/voice_recognizer/grammar_not_found', Empty, queue_size=10)
	...
	def startRecognizer(self):
		...
		rospy.Subscriber("/head/voice_recognizer/grammar_audio", AudioData, self.processAudio)
		rospy.spin()
