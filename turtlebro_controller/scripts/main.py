...
class TurtlebroController(RoboheadController):
    def _turtlebro_odom_pose2d_callback(self, msg:Pose2D):
        self.turtlebro_odom_pose2d = msg

    def __init__(self):
        super().__init__()

        # turtlebro connect
        self.turtlebro_odom_pose2d = Pose2D()
        turtlebro_topic_cmd_vel_name = rospy.get_param('~turtlebro_topic_cmd_vel_name')
        turtlebro_topic_odom_pose2d_name = rospy.get_param('~turtlebro_topic_odom_pose2d_name')
        rospy.wait_for_message(turtlebro_topic_odom_pose2d_name, Pose2D)
        self.turtlebro_pub_cmd_vel = rospy.Publisher(turtlebro_topic_cmd_vel_name, Twist, queue_size=1)
        self.turtlebro_sub_odom_pose2d = rospy.Subscriber(turtlebro_topic_odom_pose2d_name, Pose2D, self._turtlebro_odom_pose2d_callback)
        rospy.logwarn("turtlebro_controller: turtlebro connected")
    
    def start_turtlebro_controller(self):
        script_path = os.path.dirname(os.path.abspath(__file__)) + '/'
        msg = PlayAudioRequest()
        msg.is_blocking = 1
        msg.is_cycled = 0
        msg.path_to_file = script_path + "turtlebro_connected.mp3"
        self.speakers_driver_srv_PlayAudio(msg)

if __name__ == "__main__":
    rospy.init_node("turtlebro_controller")
    obj = TurtlebroController()
    obj.start_robohead_controller()
    obj.start_turtlebro_controller()
    rospy.spin()