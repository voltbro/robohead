...
def run(turtlebro_controller:TurtlebroController, cmds:str): # Обязательно наличие этой функции, именно она вызывается при голосовой команде
    ...
    prev_img = turtlebro_controller.usb_cam_image_raw

    for num in range(3,0,-1):
        str_num = str(num)
        timer_start = rospy.get_time()
        while (rospy.get_time()-timer_start)<1:
            if (prev_img!=turtlebro_controller.usb_cam_image_raw):
                prev_img = turtlebro_controller.usb_cam_image_raw
                ...
                turtlebro_controller.display_driver_pub_PlayMedia.publish(cvBridge.cv2_to_imgmsg(cv_image, encoding="bgr8")) 
    ...
    rospy.sleep(0.5)
    cvBridge = CvBridge()
    cv_image = cvBridge.imgmsg_to_cv2(turtlebro_controller.usb_cam_image_raw, "bgr8")
    cv_image = cv2.resize(cv_image, (1080, 1080))
    turtlebro_controller.display_driver_pub_PlayMedia.publish(cvBridge.cv2_to_imgmsg(cv_image, encoding="bgr8")) 
