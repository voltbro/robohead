...
class BallTracker():
    def __init__(self, mors_controller:MorsController, script_path):
        self.is_run = True
        self.mors_controller = mors_controller
    ...
    def mover(self):
        ...
        while self.is_run and not rospy.is_shutdown():
            ...
            if (rospy.get_time() - timer_start) > 2:
                msg_cmd_vel = Twist()
                self.mors_controller.mors_move(msg_cmd_vel)
                timer_start = rospy.get_time()
            ...
            if (delta_x**2+delta_y**2) > tracker_zone_radius**2:
                ...
                if not is_rotating and (abs(delta_x) <= tracker_zone_radius):
                    ...
                    velocity = min(max(0.4*abs(delta_x)/(resized_camera_resolution[0]/4), 0.1), 0.7)
                    msg_cmd_vel.angular.z = velocity*self.sign(delta_x)
                    msg_cmd_vel.linear.x = 0.05
            ...
            self.mors_controller.neck_driver_srv_NeckSetAngle(msg_neck)
            self.mors_controller.mors_move(msg_cmd_vel)
    ...
    def ball_tracker(self):
        prev_img = self.mors_controller.usb_cam_image_raw
        resized_camera_resolution = self.resized_camera_resolution
        ...
        while self.is_run and not rospy.is_shutdown():
            if (prev_img!=self.mors_controller.usb_cam_image_raw):
                prev_img = self.mors_controller.usb_cam_image_raw
                ...
                self.mors_controller.display_driver_pub_PlayMedia.publish(self.cvBridge.cv2_to_imgmsg(cv_image, encoding="bgr8"))
    ...
    def calibrate(self, duration_calibrate):
        ...
        while ((rospy.get_time()-timer_start)<duration_calibrate) and not rospy.is_shutdown():
            if (prev_img!=self.mors_controller.usb_cam_image_raw):
                ...
                self.mors_controller.display_driver_pub_PlayMedia.publish(self.cvBridge.cv2_to_imgmsg(screen_img, encoding="bgr8"))
...
def run(mors_controller:MorsController, cmds:str): # Обязательно наличие этой функции, именно она вызывается при голосовой команде
    ...
    mors_controller.mors_action(1) # Пытаемся встать, если еще не встали
    print(123123123)
    ...
    obj = BallTracker(mors_controller, script_path)
    obj.start(35, 7)
    obj.join()
